...
5. Data Preprocessing and Feature Engineering:
    ...
    - We encoded columns 'City' and 'State' with LabelEncoder(). The Random Forest Regressor model cannot interpret the categorical data from these columns, so the LabelEncoder() can convert the values to numeric datatypes.
...
9. Evaluating the model: 
    - The model was evaluated by calculating the R-squared value, Root Mean Squared Error, and Mean Absolute Error. This will help us measure how well the model performs, and the differences between the actual vs. predicted target values.
...
We can also visualize the model's output and compare the actual vs. predicted 'Item/Bottle Count' from the available data. The image below shows the actual total 'Item/Bottle Count' per month from January 2019 through December 2021 in blue, and the red line shows the model's total predicted 'Item/Bottle Count' for the last six months of 2021.
